...
/**
 * Lookup access_token on body / query
 */
function lookup(req, field) {
    return req.body && req.body[field] || req.query && req.query[field];
}
...
Strategy.prototype.authenticate = function (req, options) {
    var accessToken = lookup(this._accessTokenField);
    var refreshToken = lookup(this._refreshTokenField);
    var self = this;
...
Strategy.prototype.userProfile = function(accessToken, done) {
    this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
        ...
        try {
            ...
            profile.email = account.email || null; // account.has_email 없는 경우가 있어서 제외
